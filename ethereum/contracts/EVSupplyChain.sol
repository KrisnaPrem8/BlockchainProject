...
contract DeployContracts {
    ...
    constructor() public{
        OwnerName = "Krisna Prem";
        VehicleNumber = "AP31K3124";
    }

    function deployInsurance() public{
        address newInsurance = address(new Insurance());
        deployedInsurances.push(newInsurance);
    }

    function deployPurchase() public{
        address newPurchase = address(new EVPurchase());
        deployedPurchases.push(newPurchase);
    }
    
    function deployBattery() public{
        address newBattery = address(new BatteryMonitoring());
    ...
    function getDeployedBatteries() public view returns (address[] memory) {
        return deployedBatteries;
    }

    function getSummary() public view returns (string memory, string memory){
        return (OwnerName, VehicleNumber);
    }
}
